f1(x)  = 10^(-1.599 + 0.144*x[1] + 0.032*x[2] - 0.000111*x[2]*x[1]^2) # BDP, AC
f2(x)  = ?
f3(x)  = 10^(-1.7492 + 0.166*x[1] + 0.046*x[2] - 0.002646*x[1]*x[2]) # BDP, AC
f4(x)  = 10^(-1.1683 + 0.095*x[1] + 0.0377*x[2] - 0.0015*x[1]*x[2])   # BDP, AC
f5(x)  = 10^(0.9119 + 0.0824*x[1] + 0.0488*x[2] - 0.001599*x[1]*x[2]) # AC, HC
f6(x)  = 9.337*x[1]*x[2] - 229 # BPD, AC
f7(x)  = 10^(1.1134 + 0.1694*x[1] + 0.05845*x[2] - 0.007365*x[1]^2 - 0.000604*x[2]^2 + 0.000595*x[1]*x[2]) # BDP, AC, FL
f8(x)  = 10^(1.3596 + 0.00061*x[1]*x[2] + 0.424*x[2] + 0.174*x[3] + 0.0064*x[4] − 0.00386*x[2]*x[3]) # BDP, AC, FL, HC
f9(x)  = 10^(1.326 − 0.00326*x[1]*x[2] + 0.0107*x[3] + 0.0438*x[1] + 0.158*x[2]) # BDP, FL, HC
f10(x) = 10^(1.335 − 0.0034*x[2]*x[3] + 0.0316*x[1] + 0.0457*x[2] + 0.1623*x[3]) # BDP, AC, FL
...
f13(x) = 10^(1.6758 + 0.01707*x[2] + 0.042478*x[1] + 0.05216*x[3] + 0.01604*x[4])   # BDP, AC, FL, HC
f14(x) = 10^(1.54 + 0.15*x[1] + 0.00111*x[2]^2 − 0.0000764*x[1]*x[2]^2 + 0.05*x[3] − 0.000992*x[2]*x[3]) # BDP, AC, FL
f15(x) = 10^(1.13705 + 0.15549*x[1] + 0.04864*x[2] − 2.79682*x[1]*x[2]/1000 + 0.037769*x[3] − 4.94529*x[2]*x[3]/10000) # BDP, AC, FL
f16(x) = 10^(1.13 + 0.181864*x[1] + 0.0517505*x[2] − 3.34825*x[1]*x[2]/1000)  # BDP, AC
f17(x) = 10^(1.63 + 0.16*x[1] + 0.00111*x[2]^2 − 0.0000859*x[1]*x[2]^2)     # BDP, AC
...
f21(x) = 1.4*x[1]*x[2]*x[3] − 200                                           # BDP, AC, FL
f22(x) = 10^(2.792 + 0.108*x[2] + 0.000036*x[1]^2 − 0.00027*x[1]x[2])       # AC, FL
f23(x) = ?
f24(x) = 10^(2.1315 + 0.0056541*x[1]*x[2] − 0.00015515*x[1]*x[2]^2 + 0.000019782*x[2]^3 + 0.052594*x[1]) # BDP, AC
f25(x) = 10^(1.879 + 0.084*x[1] + 0.026*x[2])               # BDP, AC
f26(x) = -3200.40479 + 157.07186*x[2] + 15.90391*x[1]^2     # BDP, AC
...
BDP_AC_fun = [f1, f3, f4, f6, f16, f17, f24, f25, f26]
BDP_AC_index = [1, 3, 4, 6, 16, 17, 24, 25, 26]
AC_HC_fun = (f5, f20)
AC_HC_index = [5, 20]
BDP_AC_FL_fun = [f7, f10, f14, f15, f21]
BDP_AC_FL_index = [7, 10, 14, 15, 21]
BDP_AC_FL_HC_fun = [f8, f13]
BDP_AC_FL_HC_index = [8, 13]
BDP_FL_HC_fun = [f9]
BDP_FL_HC_index = [9]
...
var_color = Dict(
    ...
    :FL_T   => "#F6D891",
    :HC_T   => "#736EB8",
    :BDP_AC => "#7f848c", # https://colordesigner.io/color-mixer/?mode=rgb#F32EA1%7B0%7D-0BD976%7B0%7D-FFD92E%7B1%7D-8630BF%7B1%7D
    :BDP_FL => "#f98468",
    :BDP_HC => "#bd2fb0",
    :AC_FL  => "#85d952",
    :AC_HC  => "#49859b",
