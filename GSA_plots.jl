...
function fig_BPD_AC_1st_fig()
        ...
        row = 1
        for i in 1:length(BPD_AC_index)
                p = mod(i,2)
                if p == 0
                        ax = Axis(fig[row,p+2],
                                title = df_titles.function[BPD_AC_index[i]],
                                xticks = xtickweek2,
                                titlesize = 10
                        )
                row += 1
                ...
                sc1 = scatterlines!(ax, df_data[BPD_AC_index[i]].BPD; label = "BPD")
                sc2 = scatterlines!(ax, df_data[BPD_AC_index[i]].AC; label = "AC")
                if i == length(BPD_AC_index)
                        Legend(fig[0,:], [sc1, sc2], ["BPD", "AC"],
                        L"1st order indices of $f{(\text{BPD}, \text{AC})}$ type formulas "
                        )
                end
        end
        fig
...
function AC_FL_HC_1st_fig()
        ...
        for i in 1:length(AC_FL_HC_index)
                ...
                sc1 = scatterlines!(ax, df_data[AC_FL_HC_index[i]].AC; label = "BPD")
...
function AC_FL_HC_2nd_fig()
        ...
        for i in 1:length(AC_FL_HC_index)
                ...
                sc1 = scatterlines!(ax, df_data[AC_FL_HC_index[i]].AC_FL; label = "BPD")
...
function figBPD_AC_FL_HC_1st()
    ...
    axA = Axis(fig[1,1],
            title = L"f_8")
    scA1 = lines!(axA, DFf8r.BPD, color = "#F32EA1")
    ...
    axB = Axis(fig[2,1],
            title = L"f_{13}",
            xlabel = "Week")
    scB1 = lines!(axB, DFf13r.BPD, color = "#F32EA1")
    ...
    Legend(fig[:,2], [scA1,scA2,scA3,scA4], ["BPD", "AC", "FL", "HC"]
        )
    fig
end

fig = figBPD_AC_FL_HC_1st()
save("figure2.pdf", fig,pt_per_unit = 1)

function figBPD_AC_FL_HC_2nd()
    ...
    axA = Axis(fig[1,1],
            title = L"f_8")
    scA1 = lines!(axA, DFf8r.BPD_AC, color = "#C90130")
    scA2 = lines!(axA, DFf8r.BPD_FL, color = "#574240", linestyle = :dash)
    scA3 = lines!(axA, DFf8r.BPD_HC, color = "#BFA5A3", linestyle = :dot)
    ...
    axB = Axis(fig[2,1],
            title = L"f_{13}",
            xlabel = "Week")
    scB1 = lines!(axB, DFf13r.BPD_AC, color = "#C90130")
    scB2 = lines!(axB, DFf13r.BPD_FL, color = "#574240", linestyle = :dash)
    scB3 = lines!(axB, DFf13r.BPD_HC, color = "#BFA5A3", linestyle = :dot)
    ...
    Legend(fig[:,2], [scA1,scA2,scA3,scA4, scA5, scA6],
        ["BPD and AC", "BPD and FL", "BPD and HC", "AC and FL", "AC and HC", "FL and HC"]
        )
    fig
end

fig = figBPD_AC_FL_HC_2nd()
save("figure2.pdf", fig,pt_per_unit = 1)

function figBPD_AC_FL_HC_Total()
    size_inches = (7, 4)
    size_pt = 72 .* size_inches
    ...
    axA = Axis(fig[1,1],
            title = L"f\left(\text{BPD}, \text{AC}, \text{FL}, \text{HC} \right) = 10^{1.3596 + 0.00061\text{BPD}\cdot\text{AC} + 0.424\text{AC} + 0.174\text{FL} + 0.0064\text{HC} − 0.00386\text{AC}\cdot\text{FL}}",
            xticks = xtickweek)
    scA1 = lines!(axA, DFf8r.BPD_T)
    scA2 = lines!(axA, DFf8r.AC_T)
    scA3 = lines!(axA, DFf8r.FL_T)
    scA4 = lines!(axA, DFf8r.HC_T)

    axB = Axis(fig[2,1],
            title = L"f\left(\text{BPD}, \text{AC}, \text{FL}, \text{HC} \right) = 10^{1.6758 + 0.042478\text{BPD} + 0.01707\text{AC} + 0.05216\text{FL} + 0.01604\text{HC}}",
            xticks = xtickweek,
            xlabel = "Week")
    scB1 = lines!(axB, DFf13r.BPD_T)
    scB2 = lines!(axB, DFf13r.AC_T)
    scB3 = lines!(axB, DFf13r.FL_T)
    scB4 = lines!(axB, DFf13r.HC_T)
    Legend(fig[0,:], [scA1, scA2, scA3, scA4], ["BPD", "AC", "FL", "HC"], "Total order"
        )
    fig
end

 fig = figBPD_AC_FL_HC_Total()
save("figure2.pdf", fig,pt_per_unit = 1)
...
function fig_BPD_AC()
        ...
        for (i,df) in enumerate(dfs)
                ...
                sc1 = scatterlines!(ax, df.BPD; label = "BPD")
...
function fig_FL_HC()
        ...
        sc1 = lines!(ax, df_f9.BPD; label = "BPD")
        ...
        Legend(fig[0,:], [LineElement(color = "#F32EA1", linestyle = nothing), LineElement(color = "#0BD976", linestyle = :dash)],
                ["BPD", "AC"], L"1st order indices of $f(\text{BPD}, \text{AC})$ type formulas "
        )
